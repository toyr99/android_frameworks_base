       ...
       final Runnable SetLongPress = new Runnable () {
            public void run() {
                if (!mSearchPanelLock) {
                    ...
                    if (!MahdiActions.isActionKeyEvent(mLongList.get(mTarget))) {
                        performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);
                    }
                    sendAccessibilityEvent(AccessibilityEvent.TYPE_VIEW_LONG_CLICKED);
                    MahdiActions.processAction(mContext, mLongList.get(mTarget), true);
        ...
        public void onTrigger(View v, final int target) {
            ...
            mTarget = target;
            if (!mLongPress) {
                if (!MahdiActions.isActionKeyEvent(mIntentList.get(target))) {
                ...
                MahdiActions.processAction(mContext, mIntentList.get(target), false);
                mHandler.removeCallbacks(SetLongPress);
            }
        }
