...
public class PieMenu extends FrameLayout {
    ...
    private void getDimensions() {
        ...
        if (mPanelOrientation != Gravity.TOP)
            mSnapPoint[snapIndex++] = new SnapPoint(mWidth / 2, mSnapThickness / 2, mSnapRadius,
                    0x22, Gravity.TOP);
        ...
        mChevronPathRight = makeSlice(mPanelDegree + (mPanelOrientation != Gravity.TOP ? -5 : 3),
                mPanelDegree + 90, mInnerChevronRightRadius,
        ...
        mClockPaint
                .setTextSize(mResources.getDimensionPixelSize(R.dimen.pie_clock_size) * mPieSize);
        ...
        mStatusPaint
                .setTextSize((int) (mResources.getDimensionPixelSize(R.dimen.pie_status_size) * mPieSize));
        mStatusOffset = mResources.getDimensionPixelSize(R.dimen.pie_status_offset) * mPieSize;
        mNotificationTextSize = (int) (mResources
                .getDimensionPixelSize(R.dimen.pie_notification_size) * mPieSize);
        ...
        mOuterBatteryRadius = (int) (mInnerBatteryRadius + mResources
                .getDimensionPixelSize(R.dimen.pie_battery_increment) * mPieSize);

        mBatteryBackground.setColor(getResources().getColor(R.color.battery_background));
        mBatteryLevel = mPolicy.getBatteryLevel();
        if (mBatteryLevel <= mPolicy.lowBatteryLevel
                && mBatteryLevel > mPolicy.criticalBatteryLevel) {
            mBatteryJuice.setColor(getResources().getColor(R.color.battery_juice_low));
        } else if (mBatteryLevel <= mPolicy.criticalBatteryLevel) {
    ...
    private void getNotifications() {
        NotificationData notifData = mPanel.getBar().mNotificationData;
        if (notifData != null) {
            ...
            for (int i = 0; i < notifData.size(); i++) {
                ...
                if (entry.icon != null) {
                    ...
                    Drawable drawable = StatusBarIconView.getIcon(mContext, icon);
                    ...
                    notifictionPath.addCircle(mCenter.x, mCenter.y, mNotificationsRadius +
                            (mNotificationsRowSize * mNotificationCount)
                            + (mNotificationsRowSize - mNotificationTextSize),
                            Path.Direction.CW);
    ...
    public void onConfigurationChanged() {
        if (mStatusPanel != null)
            mStatusPanel.updatePanelConfiguration();
    }
    ...
    private void layoutPie() {
        ...
        float adjustedSweep = lesserSweepCount > 0 ? (((1 - 0.65f) * lesserSweepCount) / (itemCount - lesserSweepCount))
                : 0;
        float sweep = 0;
        float angle = 0;
        float total = 0;
        ...
        if (mOpen) {
            ...
            if (mCenterDistance > mOuterChevronRadius) {
                for (int i = 0; i < 3; i++) {
                    ...
                    Path plus = new Path();
                    plus.addRect(snap.x - len / 2, snap.y - thick / 2, snap.x + len / 2, snap.y
    ...
    private Path makeSlice(float start, float end, int outer, int inner, Point center,
            float narrow, boolean bothEnds) {
        RectF bb = new RectF(center.x - outer, center.y - outer, center.x + outer, center.y + outer);
        RectF bbi = new RectF(center.x - inner, center.y - inner, center.x + inner, center.y
                + inner);
        Path path = new Path();
    ...
    private float getPolar(float x, float y) {
        ...
        return (adjustAngle + (float) Math.atan2(
                mPanelOrientation == Gravity.TOP ? deltaY : deltaX,
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.System.getUriFor(
                    Settings.System.PA_PIE_CHEVRON_LEFT), false, this);
            resolver.registerContentObserver(Settings.System.getUriFor(
                    Settings.System.PA_PIE_CHEVRON_RIGHT), false, this);
            resolver.registerContentObserver(Settings.System.getUriFor(
